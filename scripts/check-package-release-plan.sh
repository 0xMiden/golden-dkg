...
export CRATES_IO_USER_AGENT="golden-dkg-package-release-plan"
...
while IFS=$'\t' read -r package local_version; do
    baseline_commit=""
    ...
    if ! published_package_has_library_target "$package" "$latest_version"; then
