...
**DKG configuration**:
The immutable public inputs that determine one DKG session: its session identifier, participant registry, threshold, and ordered Random/Zero instance policy. The protocol-wide Main Golden setup coefficient is not configuration input.
_Avoid_: Ceremony manifest, validator set

**Main Golden setup coefficient**:
The protocol-wide public base-field element used by the fixed Main Golden relation, derived from a fixed versioned protocol string and shared by every DKG session using that protocol version.
_Avoid_: Session beta, configuration beta, setup ID

**Participant registry**:
The validated, canonically ordered mapping from participant indexes to Golden identity public keys whose root is incorporated into the DKG configuration identity. Admission assumes each participant knows the corresponding identity secret; establishing that fact belongs to deployment rather than the current Golden protocol input.
_Avoid_: Validator set, participant list

**Dealer-proof relation**:
The fixed Main Golden statement that a dealer proves for its complete contribution. Polynomial coefficients are sampled according to the configured Random/Zero instance kinds; deriving coefficients through a VRF is not a supported policy.
