//! Transparent parameter setup benchmarks for batched BLS12-381/Jubjub eVRF
//! shapes.
//!
//! `BulletproofGens` derivation is memoized process-wide, so each sample
//! clears that cache first to keep measuring cold derivation cost.
...
fn bench_batched_setup(c: &mut Criterion) {
    let mut group = c.benchmark_group("paper eVRF batched parameter setup (bls12-381-jubjub)");
