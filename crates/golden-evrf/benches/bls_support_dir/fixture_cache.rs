//! Checked-in dealer-message fixtures shared across the BLS12-381/Jubjub
//! benches. Mirrors `crates/golden-evrf/benches/support_dir/fixture_cache.rs`
//! (Secp/Secq's), with its own on-disk namespace
...
//! shape from disk (wire-codec encoded and git-tracked), so the cost is
//! paid once and shared via git history.
//!
...
//!
//! [`regenerate_dealer_messages`] (used by the `warm_bench_fixtures_bls`
...
//!
//! A `.sha256` sidecar next to each fixture memoizes `verify_dealings`:
//! once a byte-identical file has verified, later loads trust the sidecar
//! and skip re-verifying. Editing a fixture without updating its sidecar
//! changes the digest and falls back to a real verify, so this can't mask
//! a fixture that actually changed. The shortcut is reserved for
...
const REGENERATE_HINT: &str = "cargo run --profile optimized --example warm_bench_fixtures_bls \
    --features golden-evrf/bls12-381-jubjub,golden-evrf/parallel";

fn cache_dir() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR")).join("benches/fixtures/dealer-messages-bls-jubjub")
}
...
fn build_dealer_messages(
    config: &DkgConfig<JubjubGoldenGroup>,
) -> Vec<DealerMessage<JubjubGoldenGroup>> {
...
fn write_cache(
    threshold: usize,
    n: usize,
    messages: &[DealerMessage<JubjubGoldenGroup>],
) -> io::Result<()> {
...
) -> Option<Vec<DealerMessage<JubjubGoldenGroup>>> {
...
fn load_valid(
    ...
    config: &DkgConfig<JubjubGoldenGroup>,
    force_verify: bool,
) -> Option<BTreeMap<ParticipantIndex, DealerMessage<JubjubGoldenGroup>>> {
    ...
    if !already_verified {
        ...
        verify_dealings::<JubjubGoldenGroup, BlsJubjubBackend>(&refs, config).ok()?;
...
fn expect_valid(
    ...
    config: &DkgConfig<JubjubGoldenGroup>,
    force_verify: bool,
) -> BTreeMap<ParticipantIndex, DealerMessage<JubjubGoldenGroup>> {
    let message = format!(
...
/// fixture at `benches/fixtures/dealer-messages-bls-jubjub/t{threshold}-n{n}.bin`.
///
...
/// explicitly with the `warm_bench_fixtures_bls` example rather than have
/// this silently re-prove and potentially mask a real regression. Trusts
/// the `.sha256` sidecar to skip `verify_dealings` on a byte-identical
/// fixture; use [`verify_dealer_messages`] where that shortcut isn't
/// wanted.
...
pub fn regenerate_dealer_messages(config: &DkgConfig<JubjubGoldenGroup>) -> bool {
    let n = config.registry.indexes().count();
...
pub fn cached_round1_setup(
    ...
    golden_core::DkgDealing<JubjubGoldenGroup>,
    BTreeMap<ParticipantIndex, DealerMessage<JubjubGoldenGroup>>,
) {
