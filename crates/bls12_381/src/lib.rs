...
// This is a vendored fork (see the workspace `Cargo.toml`'s
// `[patch.crates-io]` entry) that only adds `G1Affine` coordinate
// accessors; the surrounding code predates these clippy lints and is not
// hand-restyled to match a downstream consumer's lint configuration.
