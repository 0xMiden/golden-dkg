...
impl G1Affine {
    ...
    /// Returns this point's affine $x$-coordinate.
    #[inline]
    pub fn x(&self) -> Fp {
        self.x
    }

    /// Returns this point's affine $y$-coordinate.
    #[inline]
    pub fn y(&self) -> Fp {
        self.y
    }

    /// Constructs a point from its affine $(x, y)$ coordinates, without
    /// checking that it lies on the curve or in the $q$-order subgroup.
    /// **This is dangerous to call unless the coordinates are already known
    /// to satisfy those properties; otherwise, API invariants may be
    /// broken.**
    pub fn from_raw_unchecked(x: Fp, y: Fp, infinity: bool) -> Self {
        G1Affine {
            x,
            y,
            infinity: Choice::from(infinity as u8),
        }
    }
